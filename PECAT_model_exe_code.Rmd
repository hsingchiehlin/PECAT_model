...
# Figure S8A
jpeg(width = 7.5, height = 6.54, "plots/Figure.S8A.jpeg", units = "in", res=300)
...
jpeg(width = 7.5, height = 6.54, "plots/Figure.S8B.jpeg", units = "in", res=300)
pairs(log(Peff.ratio.col[,2:4]), lower.panel = NULL, 
      upper.panel = upper.panel)
dev.off()
...
abs.diff_jeju$drug.class[which(abs.diff_jeju$drug.class == 0)] <- 5
...
abs.diff_colon$drug.class[which(abs.diff_colon$drug.class == 0)] <- 5
...
abs.diff_jeju_ivive$drug.class[which(abs.diff_jeju_ivive$drug.class == 0)] <- 5
...
p5.1 <- ggplot() + 
  ...
  annotate("point", x=1.5e-4, y=55, color="#2b2d4295", size=2, shape=21) +
  annotate("text", x=1.9e-4, y=55, label=expression(paste("Drug in vivo ", "P"[eff])), hjust=0, size = 2.8) +
  annotate("point", x=1.5e-4, y=50, color="#2b2d4295", size=2, shape=16) +
  annotate("text", x=1.9e-4, y=50, label=expression(paste("Drug in vitro ", "P"[app])), hjust=0, size = 2.8) +
  annotate("point", x=1.5e-4, y=45, color="#2b2d4295", size=2.2, shape=17) +
  annotate("text", x=1.9e-4, y=45, label=expression(paste("Environmental chemical in vitro ", "P"[app])), hjust=0, size = 2.8) +
  
  annotate("text", x=1.4e-4, y=35, label="BDDCS", hjust=0, size = 2.8) +
  annotate("segment", x = 1.35e-4, xend = 1.65e-4, y = 30, yend = 30, color="#165DF995", size=1.8) +
  annotate("text", x=1.9e-4, y=30, label="Class I", hjust=0, size = 2.8) +
  annotate("segment", x = 1.35e-4, xend = 1.65e-4, y = 25, yend = 25, color="#6CBF7095", size=1.8) +
  annotate("text", x=1.9e-4, y=25, label="Class II", hjust=0, size = 2.8) +
  annotate("segment", x = 1.35e-4, xend = 1.65e-4, y = 20, yend = 20, color="#FF700095", size=1.8) +
  annotate("text", x=1.9e-4, y=20, label="Class III", hjust=0, size = 2.8) +
  annotate("segment", x = 1.35e-4, xend = 1.65e-4, y = 15, yend = 15, color="#C42A2A95", size=1.8) +
  annotate("text", x=1.9e-4, y=15, label="Class IV", hjust=0, size = 2.8) +
  annotate("segment", x = 1.35e-4, xend = 1.65e-4, y = 10, yend = 10, color="#2b2d4295", size=1.8) +
  annotate("text", x=1.9e-4, y=10, label="Not Classified", hjust=0, size = 2.8) +
...
p5.4 <- ggplot(abs.diff_jeju, aes(x= chemical, y = Diff_med)) +
  ...
  scale_color_manual(values = c("#165DF995", "#6CBF7095", "#FF700095", "#C42A2A95", "#2b2d4295"))+
...
jpeg(width = 13, height = 14, "plots/Figure.5.jpeg", units = "in", res=300)
p5.1 + p5.4 + p5.2 + p5.5 + p5.3 + p5.6 + plot_layout(widths = c(1.1/2, 0.9/2), height = c(1.12/3, 0.94/3, 0.94/3))
dev.off()
```
...
ggsave("plots/Figure.S7.jpeg", height = 3.5, width = 7.5, units="in")


# ---------- FigureS9_relationship of permeability and %abs with new data (162 drugs) ----------
...
abs.diff_jeju_ivive_newdata$drug.class[which(abs.diff_jeju_ivive_newdata$drug.class == 0)] <- 5
...
ps3.1 <- ggplot() + 
  ...
  annotate("text", x=1.4e-4, y=33, label="BDDCS", hjust=0, size = 3) +
  annotate("segment", x = 1.35e-4, xend = 1.65e-4, y = 30, yend = 30, color="#165DF995", size=1.8) +
  annotate("text", x=1.9e-4, y=30, label="Class I", hjust=0, size = 3) +
  annotate("segment", x = 1.35e-4, xend = 1.65e-4, y = 27, yend = 27, color="#6CBF7095", size=1.8) +
  annotate("text", x=1.9e-4, y=27, label="Class II", hjust=0, size = 3) +
  annotate("segment", x = 1.35e-4, xend = 1.65e-4, y = 24, yend = 24, color="#FF700095", size=1.8) +
  annotate("text", x=1.9e-4, y=24, label="Class III", hjust=0, size = 3) +
  annotate("segment", x = 1.35e-4, xend = 1.65e-4, y = 21, yend = 21, color="#C42A2A95", size=1.8) +
  annotate("text", x=1.9e-4, y=21, label="Class IV", hjust=0, size = 3) +
  annotate("segment", x = 1.35e-4, xend = 1.65e-4, y = 18, yend = 18, color="#2b2d4295", size=1.8) +
  annotate("text", x=1.9e-4, y=18, label="Not Classified", hjust=0, size = 3) +
...
ps3.2 <- ggplot(abs.diff_jeju_ivive_newdata, aes(x= chemical, y = Diff_med)) +
  geom_segment(aes(x=chemical, xend=chemical, y=Diff_LCL, yend=Diff_UCL, color=drug.class), size=0.7) +
  geom_point(aes(color = drug.class),shape = 16, size=1.8) +
  scale_color_manual(values = c("#165DF995", "#6CBF7095", "#FF700095", "#C42A2A95", "#2b2d4295"))+
...
jpeg(width = 15, height = 8.5, "plots/Figure.S9.jpeg", units = "in", res=300)
ps3.1 + ps3.2 + plot_layout(widths = c(1.3/2, 0.7/2))
dev.off()
